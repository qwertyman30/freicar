...
def change_brightness(img, value=[0.2, 0.5]):
    brightness_factor = np.random.uniform(value[0], value[1])
...
def transform(image):
    ...
    if prob <= 0.3:
        image = [torch.from_numpy(change_brightness(image[0].cpu().data.numpy()))]

    prob = np.random.uniform(0, 1)
    if prob <= 0.3:
        image = image[0].permute(1, 2, 0)
        image = cv2.GaussianBlur(image.cpu().data.numpy(), ksize=(5, 5), sigmaX=0.1, sigmaY=2.0, borderType=cv2.BORDER_DEFAULT)
...
class FreiCarDataset(data.Dataset):
    ...
    def __init__(self, data_dir, padding, split='training', load_real=False, transform=None):
    ...
    def __getitem__(self, index):
        ...
        '''
        out_dict = {}
        path = self.rgb_path_files[index]
        base = os.path.basename(path)
        file_name = os.path.splitext(base)[0]
        
        bounding_box = self.info[file_name]
        for key in bounding_box:
            bounding_box[key][0]['y'] = bounding_box[key][0]['y'] - 12
            bounding_box[key][0]['height'] = bounding_box[key][0]['height'] + 24
        
        image = loadRGB([self.rgb_path_files[index]], pad_info=self.padding)
        if self.transform:
            image = transform(image)

        out_dict['rgb'] = image
        out_dict['bbs'] = bounding_box
