...
Localizer::Localizer(std::shared_ptr<ros::NodeHandle> n) : n_(n), it_(*n) {
    ...
    marker_sub_ = n_->subscribe("/freicar_1/traffic_signs", 1, &Localizer::markerCallback, this);
    last_odo_update_ = ros::Time::now();
    pub_position_error_ = n_->advertise<std_msgs::String>("position_error", 1);
...
void Localizer::OdoCallback(const nav_msgs::OdometryConstPtr &msg) {
	...
	Eigen::Vector3f particle_pos = Eigen::Vector3f(best_particle.transform.translation().x(), best_particle.transform.translation().y(), 0.);
        ...
        std_msgs::String position_error_msg;
        position_error_msg.data =
        position_error_msg.data = std::to_string(position_error) + "_" + std::to_string(ros::Time::now().toSec() - start_time_);
        pub_position_error_.publish(position_error_msg);
    }
}
