...
class birdseyeTransformer:
    def __init__(self, homography_path, width=2, height=2.5, pix_res=500, homography_downsacling=1.0):
        self.pix_to_streetimage, self.streetimage_to_pix = self.getBirdsEyeT(homography_path, width, pix_res,
                                                                             homography_downsacling)
        self.width = width
        self.height = height
    ...
    def birdseye(self, image, interpolation_mode=cv2.INTER_LINEAR):
        bev = cv2.warpPerspective(image, self.pix_to_streetimage,
                                  dsize=(int(self.width * self.pix_res), int(self.height * self.pix_res)),
                                  flags=interpolation_mode + cv2.WARP_FILL_OUTLIERS)
        return bev

    def reverse_birdseye(self, image, s, interpolation_mode=cv2.INTER_LINEAR):
        return cv2.warpPerspective(image, self.streetimage_to_pix, dsize=(s[1], s[0]),
                                   flags=interpolation_mode + cv2.WARP_FILL_OUTLIERS)
