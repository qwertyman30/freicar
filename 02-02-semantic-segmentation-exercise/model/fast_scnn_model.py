...
class Fast_SCNN(torch.nn.Module):
    def __init__(self, input_channel, num_classes):
        ...
        # model5
        # self.global_feature_extractorReg = GlobalFeatureExtractor()
        # self.feature_fusionReg = FeatureFusionModule()

        """
        #Model 3
        self.auxlayer = nn.Sequential(
            nn.Conv2d(64, 128, 3, padding=1, bias=False),
            nn.Conv2d(128, 32, kernel_size=1, stride=1, padding=0, bias=True),
            nn.BatchNorm2d(32),
            nn.ReLU(True),
            nn.Dropout(0.1),
            nn.Conv2d(32, 1, 1)
        ...
        """
        # model3
        self.auxlayer = ClassifierReg()

        # model4
        # self.auxlayer = ClassifierRegNew()
    ...
    def forward(self, x):
        ...
        seg = torch.nn.functional.interpolate(seg, scale_factor=8, mode='bilinear')

        # model5
        # reg = self.global_feature_extractorReg(shared)
        # reg = self.feature_fusionReg(shared, reg)
        # auxout = self.auxlayer(reg)

        # auxout = self.auxlayer(shared)
        auxout = self.auxlayer(x)
        # auxout = F.interpolate(auxout, scale_factor=8, mode='bilinear', align_corners=True)
        auxout = F.interpolate(auxout, scale_factor=8, mode='bilinear', align_corners=True)
        return seg, auxout
...
class ClassifierReg(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.sconv1 = ConvBlock(in_channels=128, out_channels=128, stride=1, dilation=1, groups=128)
        self.sconv2 = ConvBlock(in_channels=128, out_channels=128, stride=1, dilation=1, groups=128)
...
class ClassifierRegNew(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.sconv1 = ConvBlock(in_channels=64, out_channels=128, stride=1, dilation=1, groups=64)
        self.sconv2 = ConvBlock(in_channels=128, out_channels=128, stride=1, dilation=1, groups=128)
        self.conv = nn.Conv2d(128, 1, kernel_size=1, stride=1, padding=0, bias=True)
